...
test("POST /api/comments", async function () {
  ...
  const response = await request(app)
    .post("/api/comments")
    .send({ user_id: userid, post_id: postid, comment: testcomment });
...
test("Patch /api/comments", async function () {
  let updatedID = carryCommentId;
  let userid = 2;
  let postid = 1;
  let testcomment = "patch testing";
  const response = await request(app).patch(`/api/comments/${updatedID}`).send({
    user_id: userid,
    post_id: postid,
    comment: testcomment,
  });
